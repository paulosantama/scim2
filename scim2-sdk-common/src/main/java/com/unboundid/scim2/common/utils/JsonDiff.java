...
public class JsonDiff {
	...
	private void processEntry(final Path parentPath,
		...
		JsonNode targetValueToReplace = targetToReplace == targetToAdd ? targetValueToAdd : targetToReplace.remove(sourceKey);
		...
		if (isSameType(sourceNode, targetValueToAdd)) {
			...
			if (targetValueToAdd.isNull() || (targetValueToAdd.isArray() && targetValueToAdd.isEmpty())) {
			...
			for (JsonNode sv : sourceNode) {
				...
				if (tv != null) {
					...
					if (sv.isObject() && tv.isObject()) {
						...
						diff(valuePath, (ObjectNode) sv, (ObjectNode) tv, (ObjectNode) tv, operations, removeMissing);
						if (!tv.isEmpty()) {
